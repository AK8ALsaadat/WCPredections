...
async function main() {
  // find Ghana or England team(s)
  const teams = await prisma.team.findMany({
    where: {
      OR: [
        { name: { contains: 'ghana', mode: 'insensitive' } },
        { shortName: { contains: 'gha', mode: 'insensitive' } },
        { name: { contains: 'england', mode: 'insensitive' } },
        { shortName: { contains: 'eng', mode: 'insensitive' } },
      ],
    },
  ...
  if (!teams || teams.length === 0) {
    console.log('No team matching "Ghana" or "England" found. Aborting.');
    return;
  }
  ...
  if (!matches || matches.length === 0) {
    console.log('No matches found for Ghana/England teams. Nothing to delete.');
    return;
  }
  const matchIds = matches.map((m) => m.id);
  console.log(`Found ${matches.length} match(es) involving Ghana/England.`);
  ...
  if (!qaUsers || qaUsers.length === 0) {
    console.log('No QA users with predictions on Ghana/England matches found.');
    return;
  }
