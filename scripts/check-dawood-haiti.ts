...
async function main() {
  // البحث عن المستخدم DAWOOD
  const user = await prisma.user.findUnique({
    where: { username: "DAWOOD" },
    select: { id: true, username: true },
  });

  if (!user) {
    console.log("User 'DAWOOD' not found.");
    return;
  }
  ...
  // البحث عن تنبؤات DAWOOD في مباريات هاييتي
  ...
  if (predictions.length === 0) {
    console.log(`\n❌ DAWOOD ليس لديه توقعات في مباريات هاييتي`);
  } else {
    console.log(`\n✅ DAWOOD لديه ${predictions.length} توقع(ات) في مباريات هاييتي:`);
    predictions.forEach((pred) => {
      console.log(
        `  - ${pred.match.homeTeam.name} vs ${pred.match.awayTeam.name}: ${pred.predictedWinner.name}`
      );
    });
  }
}
